...
function collect(creep) {
  ...
  const closestHarvester = creep.pos.findClosestByPath(FIND_MY_CREEPS, {
    filter: (c) =>
      ...
      c.store.getUsedCapacity(RESOURCE_ENERGY) / c.store.getCapacity(RESOURCE_ENERGY) > 0.25, // this 25% is arbitrary for now. It helps creeps get around to all harvesters and not ignore the ones that are just a bit further than others.
  })
